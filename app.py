...
# Safe rerun trigger at top
if st.session_state.start_exam_flag:
    st.session_state.start_exam_flag = False
...
4. Use **Previous**, **Next**, and **Save Answer** buttons to navigate.  
...
def render_section_ui(section, tab_index):
    if st.session_state.exam is not None: return  # hide tabs in exam
    with tabs[tab_index]:
        st.header(f"{section}")
        ...
        with col3:
            if st.button("▶ Start Test",key=f"{section}_start"):
                ...
                st.session_state.start_exam_flag=True
...
if st.session_state.exam:
    ...
    with c1:
        if st.button("⬅ Previous"):
            if idx>0: ex["idx"]-=1; st.session_state.exam=ex; st.experimental_rerun()
    with c2:
        if st.button("Next ➡"):
            if idx<len(ex["qs"])-1: ex["idx"]+=1; st.session_state.exam=ex; st.experimental_rerun()
    with c3:
        if st.button("💾 Save Answer"):
            st.success("Answer saved ✅")
    with c4:
        if st.button("🏁 Submit Test"):
            remaining=0  # trigger submission

    # Auto-submit when time ends or button pressed
    if remaining==0:
        # Scoring
        correct_count=0
        details=[]
